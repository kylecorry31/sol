...
docker run --rm \
  --network sonarnet \
  -v "$(pwd)/../src/main/kotlin:/usr/src" \
  sonarsource/sonar-scanner-cli \
  -Dsonar.projectKey=sol \
...
while true; do
    RESPONSE=$(curl -s -u admin:admin "http://localhost:9000/api/issues/search?componentKeys=sol&ps=$PAGE_SIZE&p=$PAGE")
...
echo ""
echo "Code analysis complete, it is available at http://localhost:9000/dashboard?id=sol  (username: admin, password: admin)"
echo ""
