 ...
 */
internal class OldAstronomyService(private val clock: Clock = Clock.systemDefaultZone()) {
    ...
    fun getMoonAzimuth(location: Coordinate): Bearing {
        return getMoonAzimuth(location, LocalDateTime.now(clock))
    }
    fun getMoonAzimuth(location: Coordinate, time: LocalDateTime): Bearing {
        return altitudeCalculator.getMoonAzimuth(location, time)
    }
    ...
    fun getSunAzimuth(location: Coordinate, time: LocalDateTime): Bearing {
        return altitudeCalculator.getSunAzimuth(location, time)
    }
