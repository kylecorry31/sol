...
internal class OceanographyTest {
    ...
    fun getTidesHarmonics() {
        ...
        val sourceString =
            if (File("temp/$station.csv").exists()) {
        ...
        val source =
            sourceString.trim().lines().drop(1).map {
                val parts = it.split(",")
                Tide(
    ...
    fun canGetTides() {
        ...
        val calculators =
            listOf(
                ...
                "LUNITIDAL (LOCAL; HIGH AND LOW)" to
                    LunitidalWaterLevelCalculator(
    ...
    private fun testCalculator(
        ...
        for (i in tides.indices) {
            ...
            val refTide =
                references
                    .filter { it.isHigh == tides[i].isHigh }
    ...
    fun generateHarmonicFile() {
        ...
        while (time.toLocalDate() <= date.plusDays(30)) {
            val level =
                HarmonicWaterLevelCalculator(harmonics).calculate(
    ...
    companion object {
        ...
        private fun time(
            ...
            minute: Int,
            zone: String,
        ): ZonedDateTime = ZonedDateTime.of(year, month, day, hour, minute, 0, 0, ZoneId.of(zone))
    }
}
