...
class EcologyTest {
    ...
    fun cumulativeGDDAccumulatesOverDates() {
        ...
        val result =
            Ecology.getCumulativeGrowingDegreeDays(
                dates,
                base,
                temperatureProvider = {
                    if (it == coldestDate) {
    ...
    fun lifecycleEventDoesNotDuplicateEventsTriggeredOnStartDate() {
        val trigger =
            object : LifecycleEventTrigger {
                override fun isTriggered(factors: LifecycleEventFactors): Boolean = !factors.date.isBefore(LocalDate.of(2024, 1, 1))
            }
    ...
    fun activePeriodsReturnsEmptyForNoEvents() {
        val result =
            Ecology.getActivePeriodsForYear(
                2024,
                emptyList(),
                "start",
                "end",
