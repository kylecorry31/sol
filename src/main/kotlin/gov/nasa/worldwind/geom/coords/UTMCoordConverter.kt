...
internal class UTMCoordConverter {
    ...
    fun convertGeodeticToUTM(
        ...
        if (errorCode == UTM_NO_ERROR.toLong()) { // no errors
            ...
            tempZone =
                if (lon < PI) {
                    (31 + ((lon * 180.0 / PI) / 6.0)).toLong()
                } else {
                    (((lon * 180.0 / PI) / 6.0) - 29).toLong()
                }
            ...
            if (UTM_Override != 0L) {
                if ((tempZone == 1L) && (UTM_Override == 60L)) {
                    tempZone = UTM_Override
                } else if ((tempZone == 60L) && (UTM_Override == 1L)) {
                    tempZone = UTM_Override
                } else if (((tempZone - 1) <= UTM_Override) && (UTM_Override <= (tempZone + 1))) {
                    tempZone =
                        UTM_Override
                } else {
                    errorCode = UTM_ZONE_OVERRIDE_ERROR.toLong()
                }
            }
            if (errorCode == UTM_NO_ERROR.toLong()) {
                if (tempZone >= 31) {
                    this.centralMeridian = (6 * tempZone - 183) * PI / 180.0
                } else {
                    this.centralMeridian = (6 * tempZone + 177) * PI / 180.0
                }
                ...
                try {
                    val TM =
                        TMCoord.fromLatLon(
                            ...
                            this.UTM_a,
                            this.UTM_f,
                            Angle.fromRadians(originLatitude),
                            Angle.fromRadians(this.centralMeridian),
                            falseEasting,
                            falseNorthing,
                            scale,
                        )
                    ...
                    if ((this.easting < MIN_EASTING) || (this.easting > MAX_EASTING)) {
                        errorCode =
                            UTM_EASTING_ERROR.toLong()
                    }
                    if ((this.northing < MIN_NORTHING) || (this.northing > MAX_NORTHING)) {
                        errorCode =
    ...
    fun convertUTMToGeodetic(
        ...
        if (Hemisphere != AVKey.SOUTH && Hemisphere != AVKey.NORTH) {
            Error_Code =
...
//            Error_Code |= UTM_EASTING_ERROR;                       //that extend into another zone
        ...
        if (Error_Code == UTM_NO_ERROR.toLong()) { // no errors
            if (Zone >= 31) {
                this.centralMeridian = ((6 * Zone - 183) * PI / 180.0 /*+ 0.00000005*/)
            } else {
                this.centralMeridian = ((6 * Zone + 177) * PI / 180.0 /*+ 0.00000005*/)
            }
