...
internal class MGRSCoordConverter {
    ...
    private fun checkZone(MGRSString: String): Long {
        ...
        if (num_digits > 2) {
            ...
            error_code = error_code or MGRS_NOZONE_WARNING.toLong()
        }
        ...
        if (MGRS == null) {
            ...
            if (error_code == MGRS_NO_ERROR.toLong()) {
                if ((MGRS.latitudeBand == LETTER_X) && ((MGRS.zone == 32) || (MGRS.zone == 34) || (MGRS.zone == 36))) {
                    ...
                    if ((MGRS.squareLetter1 < ltr2_low_value) ||
                        (MGRS.squareLetter1 > ltr2_high_value) ||
                    ...
                    if (error_code == MGRS_NO_ERROR.toLong()) {
                        ...
                        if (error_code == MGRS_NO_ERROR.toLong()) {
                            ...
                            try {
                                ...
                                if (error_code == MGRS_NO_ERROR.toLong()) {
                                    if (!(
                                            ((south - DEG_TO_RAD / divisor) <= latitude) &&
    ...
    private fun convertUPSToMGRS(
        ...
        if (AVKey.NORTH != Hemisphere && AVKey.SOUTH != Hemisphere) {
            error_code =
        ...
        if (error_code == MGRS_NO_ERROR.toLong()) {
            ...
            if (AVKey.NORTH == Hemisphere) {
                if (Easting >= TWOMIL) {
                    letters[0] = LETTER_Z.toLong()
                } else {
                    letters[0] = LETTER_Y.toLong()
                }
...
//                false_northing = UPS_Constant_Table.get(index).false_northing;
                ...
                {
                    if (Easting >= TWOMIL) {
                        letters[0] = LETTER_B.toLong()
                    } else {
                        letters[0] = LETTER_A.toLong()
                    }
...
//                false_northing = UPS_Constant_Table.get((int) letters[0]).false_northing;
    ...
    private fun convertUTMToMGRS(
        ...
        if (error_code == MGRS_NO_ERROR.toLong()) {
            ...
            grid_easting = Easting
            if (((letters[0] == LETTER_V.toLong()) && (Zone == 31L)) && (grid_easting == 500000.0)) {
                grid_easting =
    ...
    private fun getGridValues(zone: Long) {
        ...
        if (MGRS_Ellipsoid_Code.compareTo(CLARKE_1866) == 0 ||
            MGRS_Ellipsoid_Code.compareTo(CLARKE_1880) == 0 ||
            MGRS_Ellipsoid_Code.compareTo(
                BESSEL_1841,
            ) == 0 ||
    ...
    private fun getLatitudeLetter(latitude: Double): Long {
        ...
        if (lat_deg >= 72 && lat_deg < 84.5) {
            ...
            error_code = error_code or MGRS_LAT_ERROR.toLong()
        }
    ...
    private fun makeMGRSString(
        ...
        if (Zone != 0L) {
            this.mgrsString = String.format("%02d", Zone)
        } else {
            this.mgrsString = "  "
        }
        ...
        if (sEast.length > Precision) {
            sEast = sEast.substring(0, Precision.toInt() - 1)
        } else {
            var i: Int
        ...
        if (sNorth.length > Precision) {
            sNorth = sNorth.substring(0, Precision.toInt() - 1)
        } else {
            var i: Int
    ...
    companion object {
        ...
        private val latitudeBandConstants =
            arrayOf<DoubleArray?>(
