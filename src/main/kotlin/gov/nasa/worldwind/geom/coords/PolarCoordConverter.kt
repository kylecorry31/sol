...
class PolarCoordConverter
    internal constructor() {
        ...
        fun setPolarStereographicParameters(
            a: Double,
            f: Double,
            Latitude_of_True_Scale: Double,
            Longitude_Down_from_Pole: Double,
         ...
         * @return error code
         */
        fun convertGeodeticToPolarStereographic(
            Latitude: Double,
            Longitude: Double,
        ): Long {
         ...
         * @return error code
         */
        fun convertPolarStereographicToGeodetic(
            Easting: Double,
            Northing: Double,
        ): Long {
            var dy = 0.0
            ...
            if (Error_Code == POLAR_NO_ERROR) { // no errors
                if ((dy == 0.0) && (dx == 0.0)) {
                    ...
                    if (this.longitude > PI) {
                        this.longitude -= TWO_PI
                    } else if (this.longitude < -PI) {
                        this.longitude += TWO_PI
                    }

                    if (this.latitude > PI_OVER_2) {
                        // force distorted values to 90, -90 degrees
                        this.latitude = PI_OVER_2
                    } else if (this.latitude < -PI_OVER_2) {
                        this.latitude = -PI_OVER_2
                    }
