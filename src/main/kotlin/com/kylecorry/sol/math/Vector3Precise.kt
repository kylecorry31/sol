...
data class Vector3Precise(
    ...
    fun toDoubleArray(): DoubleArray = array

    fun dot(other: Vector3Precise): Double = Vector3PreciseUtils.dot(toDoubleArray(), other.toDoubleArray())

    fun magnitude(): Double = Vector3PreciseUtils.magnitude(toDoubleArray())
    ...
    companion object {
        ...
        fun from(arr: DoubleArray): Vector3Precise = Vector3Precise(arr[0], arr[1], arr[2])
    }
}
...
object Vector3PreciseUtils {
    ...
    ): DoubleArray = minus(first, project(first, planeNormal))
    ...
    fun dot(
        first: DoubleArray,
        second: DoubleArray,
    ): Double = first[0] * second[0] + first[1] * second[1] + first[2] * second[2]

    fun magnitude(arr: DoubleArray): Double = sqrt(arr[0] * arr[0] + arr[1] * arr[1] + arr[2] * arr[2])
