...
object Trigonometry {
    fun toRadians(angle: Double): Double = Math.toRadians(angle)

    fun toRadians(angle: Float): Float = Math.toRadians(angle.toDouble()).toFloat()

    fun toDegrees(angle: Double): Double = Math.toDegrees(angle)

    fun toDegrees(angle: Float): Float = Math.toDegrees(angle.toDouble()).toFloat()
    ...
    fun sinDegrees(angle: Double): Double = sin(toRadians(angle))

    fun sinDegrees(angle: Float): Float = sin(toRadians(angle))

    fun cosDegrees(angle: Double): Double = cos(toRadians(angle))

    fun cosDegrees(angle: Float): Float = cos(toRadians(angle))

    fun tanDegrees(angle: Double): Double = tan(toRadians(angle))

    fun tanDegrees(angle: Float): Float = tan(toRadians(angle))

    fun normalizeAngle(angle: Float): Float = Arithmetic.wrap(angle, 0f, 360f) % 360

    fun normalizeAngle(angle: Double): Double = Arithmetic.wrap(angle, 0.0, 360.0) % 360
    ...
    fun connect(
        first: Vector2,
        second: Vector2,
        approximateFrequency: Float? = null,
    ): Waveform {
        ...
        if (approximateFrequency != null) {
            ...
            frequency =
                if (abs(approximateFrequency - below) < abs(approximateFrequency - above)) {
                    below
                } else {
                    above
                }
    ...
    fun getRangeY(waveform: Waveform): Range<Float> =
        Range(-waveform.amplitude + waveform.verticalShift, waveform.amplitude + waveform.verticalShift)
