...
class NewtonPolynomialRegression(
    ...
    private fun getPolynomialTerms(
        ...
        for (i in n - 2 downTo 0) {
            polynomial = Polynomial.fromCoefficients(coefficients[i]) + polynomial *
                Polynomial(
                    listOf(
                        PolynomialTerm(1f, 1),
    ...
    private fun getDividedDifferenceCoefficients(points: List<Vector2>): Array<Float> {
        val n = order + 1
        val a = Array(n) { 0f }
        for (i in 0 until n) {
            a[i] = points[i].y
