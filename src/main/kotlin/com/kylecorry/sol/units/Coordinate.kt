...
data class Coordinate(
    ...
    fun plus(
        distance: Distance,
        bearing: Bearing,
    ): Coordinate = plus(distance.meters().value.toDouble(), bearing)
    ...
    companion object {
        ...
        fun toLongitude(degrees: Double): Double = Arithmetic.wrap(degrees, -180.0, 180.0)

        fun constrained(
            latitude: Double,
            longitude: Double,
        ): Coordinate = Coordinate(latitude.coerceIn(-90.0, 90.0), toLongitude(longitude))

        private val defaultFormats =
            listOf(
        ...
        ): Coordinate? = formats.firstNotNullOfOrNull { it.parse(location) }

        internal fun isValidLongitude(longitude: Double): Boolean = longitude.absoluteValue <= 180

        internal fun isValidLatitude(latitude: Double): Boolean = latitude.absoluteValue <= 90
    }
}
