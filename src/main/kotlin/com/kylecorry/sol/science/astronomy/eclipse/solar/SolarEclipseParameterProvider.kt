...
internal class SolarEclipseParameterProvider {
    ...
    private fun getNextSolarEclipse(ut: UniversalTime): SolarEclipseParameters {
        ...
        do {
            ...
            F =
                Trigonometry.normalizeAngle(
                    160.7108 + 390.67050284 * k - 0.0016118 * power(T, 2) - 0.00000227 *
                        power(
                            T,
                            3,
        ...
        val MPrime =
            Trigonometry.normalizeAngle(
                201.5643 + 385.81693528 * k + 0.0107582 * power(T, 2) + 0.00001238 *
                    power(
                        T,
                        3,
        ...
        val omega =
            Trigonometry.normalizeAngle(
                124.7746 - 1.56375588 * k + 0.0020672 *
                    power(
                        T,
                        2,
        ...
        val table = table54_1()
        for (row in table) {
            correction += (row[0] / 10000.0) * (if (row[1] == 0) 1.0 else E) *
                sinDegrees(
        ...
        for (row in tableP) {
            p += (row[0] / 10000.0) * (if (row[1] == 0) 1.0 else E) *
                sinDegrees(
        ...
        for (row in tableQ) {
            q += (row[0] / 10000.0) * (if (row[1] == 0) 1.0 else E) *
                cosDegrees(
        ...
        val u =
            0.0059 + 0.0046 * E * cosDegrees(M) - 0.0182 * cosDegrees(MPrime) + 0.0004 *
                cosDegrees(
                    2 * MPrime,
    ...
    private fun getJDEOfMeanMoonPhase(k: Double): Double {
        val T = k / 1236.85
        return 2451550.09766 + 29.530588861 * k + 0.00015437 * power(T, 2) - 0.00000015 *
            power(
                T,
                3,
