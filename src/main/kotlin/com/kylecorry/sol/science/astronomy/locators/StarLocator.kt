...
internal class StarLocator(
    ...
    override fun getCoordinates(ut: UniversalTime): EquatorialCoordinate {
        ...
        val B =
            cosDegrees(value3) * cosDegrees(declination) * cosDegrees(rightAscension + value1) - sinDegrees(value3) *
                sinDegrees(
                    declination,
                )
        val C =
            sinDegrees(value3) * cosDegrees(declination) * cosDegrees(rightAscension + value1) + cosDegrees(value3) *
                sinDegrees(
                    declination,
                )
        ...
        val dec =
            if (declination.absoluteValue > 89.0) {
                acos(hypot(A, B))
            } else {
                asin(C)
            }.toDegrees()
    ...
    override fun getDistance(ut: UniversalTime): Distance? = null
}
