...
internal class PlanetLocator(
    ...
    override fun getCoordinates(ut: UniversalTime): EquatorialCoordinate {
        ...
        val longitudeAdjustment =
            normalizeAngle(
                planet.eclipticLongitudeAscendingNodeEpoch +
                    atan2(
                        sinDegrees(planetPosition.first.eclipticLongitude - planet.eclipticLongitudeAscendingNodeEpoch) *
                            cosDegrees(
                                planet.inclination,
                            ),
        ...
        val geocentricEclipticLongitude =
            normalizeAngle(
                if (planet.order <= Planet.Earth.order) {
                    180 + earthPosition.first.eclipticLongitude +
                        atan2(
                            planetPosition.second * cosDegrees(planetPosition.first.eclipticLatitude) *
                                sinDegrees(earthPosition.first.eclipticLongitude - longitudeAdjustment),
                            earthPosition.second - planetPosition.second * cosDegrees(planetPosition.first.eclipticLatitude) *
                                cosDegrees(
                    ...
                    longitudeAdjustment +
                        atan2(
                            ...
                            planetPosition.second * cosDegrees(planetPosition.first.eclipticLatitude) - earthPosition.second *
                                cosDegrees(
                                    earthPosition.first.eclipticLongitude - longitudeAdjustment,
                                ),
                        ).toDegrees()
        ...
        val geocentricEclipticLatitude =
            normalizeAngle(
                atan2(
                    planetPosition.second * cosDegrees(planetPosition.first.eclipticLatitude) *
                        tanDegrees(planetPosition.first.eclipticLatitude) *
                        sinDegrees(
    ...
    private fun getDistanceAU(
        ...
        sqrt(
            square(earthPosition.second) + square(planetPosition.second) - 2 * earthPosition.second * planetPosition.second *
                cosDegrees(
                    planetPosition.first.eclipticLongitude - earthPosition.first.eclipticLongitude,
                ),
        )

    private fun getHeliocentricPosition(
        planet: Planet,
        ut: UniversalTime,
    ): Pair<EclipticCoordinate, Double> {
        val meanAnomaly =
            OrbitalMath.getMeanAnomaly(
        ...
        val coordinate =
            OrbitalMath.getEclipticCoordinate(
                trueAnomaly,
                planet.eclipticLongitudePerihelion,
                planet.eclipticLongitudeAscendingNodeEpoch,
                planet.inclination,
