...
internal object ZambrettiForecaster : Forecaster {
    ...
    private fun forecast(
        ...
        val z =
            if (tendency.amount < changeThreshold) {
                144 - 0.13 * hpa
    ...
    private fun getCurrentConditions(
        ...
        ForecastHelper.addSecondaryConditions(
    ...
    private fun getLaterConditions(
        z: Double,
        dailyTemperatureRange: Range<Temperature>?,
    ): List<WeatherCondition> = ForecastHelper.addSecondaryConditions(conditions[z.toInt()]?.later ?: emptyList(), dailyTemperatureRange)

    private data class Conditions(
        val current: List<WeatherCondition>,
        val later: List<WeatherCondition>,
    )
}
