...
object Physics {
    fun fallHeight(
        time: Duration,
        gravity: Float = Geophysics.GRAVITY,
    ): Distance {
    ...
    fun isMetal(
        ...
        threshold: Float = 65f,
    ): Boolean = isMetal(magneticField.magnitude(), threshold)

    fun isMetal(
        fieldStrength: Float,
        threshold: Float = 65f,
    ): Boolean = fieldStrength >= threshold
    ...
    fun removeGeomagneticField(
        ...
        val updatedGeo =
            orientationChange?.rotate(geomagneticField)
