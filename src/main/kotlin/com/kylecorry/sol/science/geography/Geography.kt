...
object Geography {
    ...
    fun getECEF(location: Location): Vector3 {
        ...
        val x = (n + location.elevation.meters().amount) * cos(lat) * cos(lon)
        val y = (n + location.elevation.meters().amount) * cos(lat) * sin(lon)
        val z = (n * (1 - square(e)) + location.elevation.meters().amount) * sin(lat)
    ...
    fun trilaterate(
        ...
        val errors = readings.map { it.radius.convertTo(Distance.NauticalMiles).amount / 60f }
    ...
    private fun trilaterate2(readings: List<Geofence>): List<Coordinate> {
        ...
        val radii = readings.map { (it.radius.convertTo(Distance.NauticalMiles).amount / 60f).toRadians() }
